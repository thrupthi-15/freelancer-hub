...
def view_posts():
    ...
    if search:
        like = f"%{search}%"
        cur.execute("""SELECT p.*, u.name FROM projects p JOIN users u ON p.employer_id=u.id
...
def my_tasks():
    cur = mysql.connection.cursor()
    cur.execute("""SELECT at.id, p.title, p.description, p.skills_required, p.deadline,
                          p.attachment, at.bid_amount, at.progress, at.assigned_at, u.name
                   FROM assigned_tasks at
                   JOIN projects p ON at.project_id=p.id
                   JOIN users u ON at.employer_id=u.id
                   WHERE at.employee_id=%s""", (session['user_id'],))
    tasks = cur.fetchall()
    cur.close()
...
def view_payment(task_id):
    ...
    cur.execute("""SELECT at.bid_amount, COALESCE(SUM(pay.amount),0)
                   ...
                   GROUP BY at.id""", (task_id, session['user_id']))
    info = cur.fetchone()
...
# ---- Shared: serve PDF attachments (accessible by employees too) ----
@employee_bp.route('/attachment/<filename>')
@login_required
