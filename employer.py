...
def view_bids(pid):
    ...
    cur.execute("""SELECT u.id, u.name, u.gender, u.email, u.phone, u.profile_image, b.bid_amount
                   FROM bids b JOIN users u ON b.employee_id=u.id
                   WHERE b.project_id=%s ORDER BY b.bid_amount ASC""", (pid,))
    bids = cur.fetchall()
    cur.close()
...
def view_assigned():
    cur = mysql.connection.cursor()
    cur.execute("""SELECT at.id, p.title, p.description, p.skills_required, p.budget, p.deadline,
                          p.attachment, p.posted_at, u.name, u.email, u.phone,
                          at.bid_amount, at.progress, at.assigned_at
                   FROM assigned_tasks at
                   JOIN projects p ON at.project_id=p.id
                   JOIN users u ON at.employee_id=u.id
                   WHERE at.employer_id=%s ORDER BY at.assigned_at DESC""", (session['user_id'],))
    tasks = cur.fetchall()
    cur.close()
...
def view_status(task_id):
    cur = mysql.connection.cursor()
    cur.execute("""SELECT at.progress, at.progress_note, p.title, u.name
                   FROM assigned_tasks at
                   JOIN projects p ON at.project_id=p.id
                   JOIN users u ON at.employee_id=u.id
                   WHERE at.id=%s AND at.employer_id=%s""", (task_id, session['user_id']))
    task = cur.fetchone()
    cur.close()
...
def payment(task_id):
    ...
    cur.execute("""SELECT at.bid_amount, COALESCE(SUM(pay.amount),0)
                   FROM assigned_tasks at
                   LEFT JOIN payments pay ON pay.task_id=at.id
                   WHERE at.id=%s GROUP BY at.id""", (task_id,))
    info = cur.fetchone()
